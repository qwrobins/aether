...
describe('registerTransferHandlers', () => {
  ...
  it('rejects single-file SFTP downloads with traversal in the destination path', async () => {
    ...
    await expect(
      handlers.get(IpcChannels.TRANSFER_START)?.({}, createRequest({
        direction: 'download',
        connectionType: 'sftp',
        sourcePath: '/remote/authorized_keys',
  ...
  it('rejects single-file S3 downloads with traversal in the destination path', async () => {
    const { handlers } = await createIpcHandlerSetup();
    ...
    await expect(
      handlers.get(IpcChannels.TRANSFER_START)?.({}, createRequest({
        direction: 'download',
        connectionType: 's3',
        sourcePath: 'photos/authorized_keys',
  ...
  it('allows single-file SFTP downloads with traversal only in the remote path', async () => {
    // Remote paths containing '..' are legal on the server; the security
    // invariant is that the local destination is contained and name-matched.
    const client = { kind: 'sftp-client', stat: vi.fn().mockResolvedValue({ isDirectory: false }) };
    getSftpClient.mockReturnValue(client);
    ...
    await handlers.get(IpcChannels.TRANSFER_START)?.({}, createRequest({
      ...
      destinationPath: '/home/user/Downloads/authorized_keys',
      bucket: undefined,
      isDirectory: false,
    }));
